...
import { Plus, Minus, Package, CreditCard as Edit2, Trash2, X, Eye, EyeOff, Lock, Rocket, CheckCircle2 } from 'lucide-react';
import ImageUpload from '../../components/ImageUpload';
import { sortBundlesWithAlphaBetaGamma, parseBundleItemsList, updateItemQuantityInString } from '../../lib/bundleUtils';
...
export default function AdminBundles() {
  ...
  // Auto-calculate price from selected inventory items (with smart quantity parsing)
  const recalculatePrice = (itemsString: string) => {
    const parsed = parseBundleItemsList(itemsString, inventoryItems);
    return parsed.reduce((sum, item) => sum + item.totalPrice, 0);
  };
          ...
          <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-lg">
            ...
            >
                ...
                {inventoryItems.length > 0 && (
                  <div className="mt-2 p-3 bg-slate-50 border border-slate-200 rounded-xl">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-xs font-bold text-slate-700">Quick Add & Quantity Stepper:</p>
                      <span className="text-[11px] text-slate-500">Tap + / − to adjust items</span>
                    </div>
                    <div className="flex flex-wrap gap-2 max-h-44 overflow-y-auto pr-1">
                      {inventoryItems.map((item, idx) => {
                        const parsedItems = parseBundleItemsList(formData.items, inventoryItems);
                        const foundInBundle = parsedItems.find(p => {
                          if (p.inventoryItem) {
                            return p.inventoryItem.name.toLowerCase() === item.name.toLowerCase();
                          }
                          return p.name.toLowerCase() === item.name.toLowerCase();
                        });
                        const qty = foundInBundle ? foundInBundle.quantity : 0;
                          ...
                          >
                            {!isSelected ? (
                              <button
                                type="button"
                                onClick={() => {
                                  const newItemsStr = updateItemQuantityInString(formData.items, item.name, 1, inventoryItems);
                                ...
                                className="inline-flex items-center px-2.5 py-1.5 hover:text-emerald-700"
                              >
                                <Plus className="w-3 h-3 mr-1 text-slate-400" />
                                <span>{item.name}</span>
                                <span className="ml-1.5 text-[10px] text-slate-400 font-mono font-normal">
                                  GH₵{Number(item.price).toFixed(0)}
                                </span>
                              </button>
                            ) : (
                              <div className="flex items-center px-1.5 py-1 gap-1">
                                <span className="font-semibold text-emerald-800 text-xs px-1">
                                  {item.name}
                                </span>
                                <div className="flex items-center bg-white rounded-md border border-emerald-300/80 shadow-2xs ml-1">
                                  <button
                                    type="button"
                                    onClick={() => {
                                      const newItemsStr = updateItemQuantityInString(formData.items, item.name, -1, inventoryItems);
                                    ...
                                    className="p-1 text-emerald-700 hover:bg-emerald-100 rounded-l transition"
                                    title="Decrease quantity"
                                  >
                                    <Minus className="w-3 h-3" />
                                  </button>
                                  <span className="px-2 text-xs font-bold text-emerald-900 min-w-[20px] text-center font-mono">
                                    {qty}
                                  </span>
                                  <button
                                    type="button"
                                    ...
                                    className="p-1 text-emerald-700 hover:bg-emerald-100 rounded-r transition"
                                    title="Increase quantity"
                                  >
                                    <Plus className="w-3 h-3" />
                                  </button>
                                </div>
                                <span className="text-[10px] text-emerald-700 font-mono font-bold ml-1">
                                  GH₵{(qty * Number(item.price)).toFixed(0)}
                                </span>
                              </div>
                            )}
                          </div>
              ...
              <div>
                ...
                {(() => {
                  const parsed = parseBundleItemsList(formData.items, inventoryItems);
                  const matched = parsed.filter(p => p.inventoryItem);
                  const totalNetCost = matched.reduce((s, m) => s + m.totalPrice, 0);

                  return (
                    <>
                      {matched.length > 0 && (
                        <div className="mb-3 p-3 bg-emerald-50/80 border border-emerald-200/80 rounded-xl shadow-2xs">
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-[10px] font-bold text-emerald-800 uppercase tracking-wider flex items-center gap-1">
                              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                              Auto-calculated Breakdown ({matched.reduce((acc, it) => acc + it.quantity, 0)} items)
                            </p>
                            <span className="text-[10px] text-emerald-700 font-bold bg-emerald-100/90 px-2 py-0.5 rounded-full">
                              Base Net Cost
                            </span>
                          </div>
                          ...
                          <div className="space-y-1.5 divide-y divide-emerald-100">
                            {matched.map((m, i) => (
                              <div key={i} className="flex justify-between items-center text-xs text-emerald-900 pt-1.5">
                                <span className="font-medium flex items-center gap-1.5">
                                  {m.quantity > 1 && (
                                    <span className="bg-emerald-200/70 text-emerald-800 font-bold px-1.5 py-0.2 rounded text-[11px]">
                                      {m.quantity}×
                                    </span>
                                  )}
                                  <span>{m.name}</span>
                                  {m.quantity > 1 && (
                                    <span className="text-[10px] text-emerald-600 font-normal">
                                      (@ GH₵ {m.unitPrice.toFixed(2)})
                                    </span>
                                  )}
                                </span>
                                <span className="font-bold font-mono">
                                  GH₵ {m.totalPrice.toFixed(2)}
                                </span>
                              </div>
                            ))}
                          </div>

                          <div className="flex justify-between items-center text-xs font-black text-emerald-950 pt-2 mt-2 border-t border-emerald-300">
                            <span>Total Inventory Cost:</span>
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-mono text-emerald-800">
                                GH₵ {totalNetCost.toFixed(2)}
                              </span>
                              <button
                                type="button"
                                onClick={() => setFormData(prev => ({ ...prev, price: totalNetCost.toFixed(2) }))}
                                className="text-[10px] bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-2 py-1 rounded-md transition shadow-2xs"
                                title="Set package price to this inventory total"
                              >
                                Apply as Price
                              </button>
                            </div>
                      ...
                      <PaystackFeeCalculator
                        ...
                        suggestedBasePrice={totalNetCost > 0 ? totalNetCost.toFixed(2) : undefined}
                ...
                <input
                  ...
                  className="w-full px-4 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none bg-slate-50 font-bold text-slate-900"
                  required
                  placeholder="0.00"
                />
                <p className="text-[10px] text-slate-400 mt-1">Final price charged to students & guests at checkout (including Paystack charges).</p>
              </div>
